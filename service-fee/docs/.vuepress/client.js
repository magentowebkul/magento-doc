...
import ExplainCode from "./components/ExplainCode.vue";
...
export default defineClientConfig({
    enhance({ app }) {
        app.component("ExplainCode", ExplainCode);
        ...
        if (typeof window !== "undefined") {
            requestAnimationFrame(mountPerPageFeatures);
        }
...
const ensureMermaidRendered = async () => {
    ...
    mermaidWrappers.forEach((wrapper) => {
        const svg = wrapper.querySelector(".mermaid-content svg");
        const g = svg?.querySelector("g");
        if (svg && (!g || g.children.length === 0)) {
            needsRender = true;
        }
    });
    ...
    try {
        ...
        for (const wrapper of mermaidWrappers) {
            ...
            const g = svg?.querySelector("g");
            if (svg && (!g || g.children.length === 0)) {
                const btn = wrapper.parentElement?.querySelector(".preview-button") || wrapper.querySelector(".preview-button");
                const codeProp = wrapper.__vueParentComponent?.props?.code || btn?.__vueParentComponent?.props?.code;

                if (codeProp) {
                    const rawCode = decodeData(codeProp);
                    const uniqueId = `mermaid-auto-${Math.random().toString(36).slice(2)}`;
                    const res = await mermaid.render(uniqueId, rawCode);

                    const contentEl = wrapper.querySelector(".mermaid-content") || wrapper;
                    contentEl.innerHTML = res.svg;
                }
            }
        }
    } catch (e) {
        console.warn("Mermaid auto-render:", e);
    }
};

if (typeof document !== "undefined" && !window.__mermaidPreviewHandlerBound) {
    window.__mermaidPreviewHandlerBound = true;
    document.addEventListener("click", (e) => {
        const btn = e.target.closest(".preview-button");
        if (btn) {
            const wrapper = btn.closest(".mermaid-actions")?.parentElement?.querySelector(".mermaid-wrapper") || btn.closest(".mermaid-wrapper");
            const content = wrapper?.querySelector(".mermaid-content") || wrapper;
            const svgHtml = content?.querySelector("svg")?.outerHTML || content?.innerHTML;
            if (svgHtml) {
                setTimeout(() => {
                    const modal = document.querySelector(".mermaid-preview");
                    if (modal) {
                        const uniqueId = `mermaid-modal-svg-${Date.now()}`;
                        modal.innerHTML = svgHtml
                            .replace(/id="mermaid-[^"]*"/g, `id="${uniqueId}"`)
                            .replace(/#mermaid-[a-zA-Z0-9_-]+/g, `#${uniqueId}`);
                    }
                }, 10);
            }
        }
...
if (typeof document !== "undefined" && !window.__mermaidDownloadHandlerBound) {
    window.__mermaidDownloadHandlerBound = true;
    document.addEventListener("click", (e) => {
        ...
        if (btn) {
            ...
            const wrapper = btn.closest(".mermaid-actions")?.parentElement?.querySelector(".mermaid-wrapper") || btn.closest(".mermaid-wrapper");
            const svg = wrapper?.querySelector("svg") || document.querySelector(".mermaid-preview svg");
            if (!svg) return;

            const clone = svg.cloneNode(true);
            const isDark = document.documentElement.getAttribute("data-theme") === "dark" || document.body.classList.contains("dark");
            const bgColor = isDark ? "#1e1e1e" : "#ffffff";
            const nodeFill = isDark ? "#389d70" : "#4abf8a";
            const labelTextColor = isDark ? "#e2e8f0" : "#333333";
            const lineStroke = isDark ? "#d3d3d3" : "#333333";

            clone.setAttribute("style", `background-color: ${bgColor} !important; border-radius: 8px; ${clone.getAttribute("style") || ""}`);

            const styleEl = document.createElementNS("http://www.w3.org/2000/svg", "style");
            styleEl.textContent = `
                .node rect, .clusters rect, rect.note, .actor { rx: 8px !important; ry: 8px !important; fill: ${nodeFill} !important; stroke: ${nodeFill} !important; }
                .node polygon { fill: ${nodeFill} !important; stroke: ${nodeFill} !important; }
                .edgePath path, .flowchart-link { stroke: ${lineStroke} !important; stroke-width: 1.5px !important; }
                .marker { fill: ${lineStroke} !important; stroke: ${lineStroke} !important; }
                .node foreignObject { overflow: visible !important; }
                .node foreignObject > div { display: flex !important; align-items: center !important; justify-content: center !important; width: 100% !important; height: 100% !important; min-height: 100% !important; margin: 0 !important; padding: 0 !important; box-sizing: border-box !important; }
                .node foreignObject p, .node foreignObject span, .nodeLabel { display: flex !important; align-items: center !important; justify-content: center !important; text-align: center !important; color: #ffffff !important; fill: #ffffff !important; font-weight: 400 !important; font-size: 13px !important; line-height: 1.3 !important; margin: 0 !important; padding: 0 !important; width: 100% !important; }
                .edgeLabel, .edgeLabel g, .edgeLabel foreignObject { overflow: visible !important; display: flex !important; align-items: center !important; justify-content: center !important; }
                .edgeLabel .labelBkg, .labelBkg, .edgeLabel foreignObject div, .edgeLabel span.edgeLabel { display: inline-flex !important; align-items: center !important; justify-content: center !important; text-align: center !important; background-color: transparent !important; background: transparent !important; border: none !important; box-shadow: none !important; padding: 0 4px !important; margin: 0 !important; position: relative !important; overflow: visible !important; color: ${labelTextColor} !important; fill: ${labelTextColor} !important; }
                .edgeLabel rect { fill: transparent !important; stroke: none !important; display: none !important; }
                .edgeLabel *, .labelBkg * { background-color: transparent !important; background: transparent !important; border: none !important; box-shadow: none !important; }
                .edgeLabel p, .edgeLabel span { display: inline-flex !important; align-items: center !important; justify-content: center !important; text-align: center !important; color: ${labelTextColor} !important; fill: ${labelTextColor} !important; font-weight: 500 !important; font-size: 11px !important; line-height: 1.2 !important; margin: 0 !important; padding: 0 !important; }
            `;
            clone.insertBefore(styleEl, clone.firstChild);

            const svgHtml = clone.outerHTML;
            const dataUrl = `data:image/svg+xml;charset=utf8,${svgHtml
                .replace(/<br>/g, "<br />")
                .replace(/%/g, "%25")
                .replace(/"/g, "%22")
                .replace(/'/g, "%27")
            ...
            link.setAttribute("download", `${btn.title || "mermaid-chart"}.svg`);
...
const enhanceAlertBlockquotes = () => {
    ...
    blockquotes.forEach((bq) => {
        ...
        if (text.includes("[!IMPORTANT]")) {
            bq.classList.add("doc-alert", "doc-alert--important");
            bq.innerHTML = text.replace(/\[!IMPORTANT\]\s*<br\s*\/?>?/gi, '<strong class="doc-alert__title">Important</strong>').replace(/\[!IMPORTANT\]/gi, '<strong class="doc-alert__title">Important</strong>');
        } else if (text.includes("[!NOTE]")) {
            bq.classList.add("doc-alert", "doc-alert--note");
            bq.innerHTML = text.replace(/\[!NOTE\]\s*<br\s*\/?>?/gi, '<strong class="doc-alert__title">Note</strong>').replace(/\[!NOTE\]/gi, '<strong class="doc-alert__title">Note</strong>');
        } else if (text.includes("[!WARNING]")) {
            bq.classList.add("doc-alert", "doc-alert--warning");
            bq.innerHTML = text.replace(/\[!WARNING\]\s*<br\s*\/?>?/gi, '<strong class="doc-alert__title">Warning</strong>').replace(/\[!WARNING\]/gi, '<strong class="doc-alert__title">Warning</strong>');
        } else if (text.includes("[!TIP]")) {
            bq.classList.add("doc-alert", "doc-alert--tip");
            bq.innerHTML = text.replace(/\[!TIP\]\s*<br\s*\/?>?/gi, '<strong class="doc-alert__title">Tip</strong>').replace(/\[!TIP\]/gi, '<strong class="doc-alert__title">Tip</strong>');
        } else if (text.includes("[!CAUTION]")) {
            bq.classList.add("doc-alert", "doc-alert--caution");
            bq.innerHTML = text.replace(/\[!CAUTION\]\s*<br\s*\/?>?/gi, '<strong class="doc-alert__title">Caution</strong>').replace(/\[!CAUTION\]/gi, '<strong class="doc-alert__title">Caution</strong>');
        } else {
            bq.classList.add("doc-blockquote");
        }
    });
};
